/* <lm-makeup-viewer> — Longmotive makeup-water pump-room inspector.
   Loads uploads/3d/makeup-water-pump-room.glb (duty/standby booster pump skid,
   blue pressure vessel, 3 control cabinets, overhead fire main). Raycast hits
   group by MW_* mesh-name families so a click selects the whole item. */
(function(){
const ESM='https://cdn.jsdelivr.net/npm/three@0.160.0';
const GLB='uploads/3d/makeup-water-pump-room.glb';
const POSTER='uploads/3d/makeup-water-hero.png';
const SEC={
  PS:{name:'PUMP SKID',type:'Booster pump set',devices:'Duty + standby vertical multistage pumps · manifold'},
  TK:{name:'PRESSURE TANK',type:'Bladder pressure vessel',devices:'Makeup-water expansion / pressure vessel'},
  C1:{name:'CONTROL PANEL',type:'Pump control cabinet',devices:'Gauges · run / trip / standby lamps'},
  C2:{name:'CABINET 2',type:'Electrical cabinet',devices:'Distribution & starter cabinet'},
  C3:{name:'CABINET 3',type:'Electrical cabinet',devices:'Distribution & starter cabinet'},
  OH:{name:'OVERHEAD SERVICES',type:'Ceiling services',devices:'Fire main · luminaire · supports'}
};
...
class LMMakeupViewer extends HTMLElement{
  connectedCallback(){
  ...
  _init(THREE,GLTFLoader,RoomEnvironment){
    ...
    new GLTFLoader().load(GLB,(gltf)=>{
      ...
      scene.add(root);
      // glow the luminaire tube + drop a light pool beneath it
      let tube=null;
      root.traverse(o=>{ if(o.isMesh&&o.material&&o.material.emissive&&/MW_Tube/.test(o.material.name||'')){o.material.emissive.setHex(0xffffff);o.material.emissiveIntensity=2.2;o.material.toneMapped=false;tube=tube||o;} });
      if(tube){
        const lb=new THREE.Box3().setFromObject(tube),lc=lb.getCenter(new THREE.Vector3());
        const p=new THREE.PointLight(0xf2f6ff,26,0,2);
        p.position.copy(lc);p.position.y-=.14;scene.add(p);
      }
      const bb=new THREE.Box3().setFromObject(root);
      const c=bb.getCenter(new THREE.Vector3()),sz=bb.getSize(new THREE.Vector3());
      C.copy(c);
      const span=Math.max(sz.x,sz.z);
      R_MIN=span*.18;R_MAX=span*2.6;
      VIEWS.overview.t.copy(c);VIEWS.overview.r=span*2.0;
    ...
    /* ---------------- camera rig: orbit clamped to the open corner ---------------- */
...
if(!customElements.get('lm-makeup-viewer'))customElements.define('lm-makeup-viewer',LMMakeupViewer);
})();
